...
TYPE
  ...
  TXmlParser = CLASS                             // --- Internal Properties and Methods
               ...
               PUBLIC
                 ...
                 FUNCTION  TranslateEncoding  (CONST sSource : STRING) : STRING;     VIRTUAL;
...
FUNCTION  TXmlParser.TranslateEncoding  (CONST sSource : STRING) : STRING;
...
BEGIN
  IF CurEncoding = 'UTF-8'
    THEN Result := Utf8ToAnsi (sSource)
    ELSE Result := sSource;
END;
...
FUNCTION  TNvpList.GetName (Index : INTEGER) : STRING;
BEGIN
  IF (Index < 0) OR (Index >= Count)
    THEN Result := ''
...
PROCEDURE TElemList.Add (oNode : TElemDef);
VAR
  I : INTEGER;
BEGIN
  FOR I := Count-1 DOWNTO 0 DO
    IF oNode.Name > TElemDef (Items [I]).Name THEN BEGIN
      Insert (I+1, oNode);
      EXIT;
      END;
  Insert (0, oNode);
...
TYPE
  TScannerXmlParser = CLASS (TXmlParser)
                       ...
                       FUNCTION  TranslateEncoding  (CONST sSource : STRING) : STRING;  OVERRIDE;
...
FUNCTION  TScannerXmlParser.TranslateEncoding  (CONST sSource : STRING) : STRING;
BEGIN
  IF Assigned (Scanner.FOnTranslateEncoding)
    THEN Result := Scanner.FOnTranslateEncoding (Scanner, CurEncoding, sSource)
    ELSE Result := INHERITED TranslateEncoding (sSource);
END;
